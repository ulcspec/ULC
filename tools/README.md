...
- `validator/` is the reference command-line validator and index builder (ship name: `ulc`). Written in Go and built around [`santhosh-tekuri/jsonschema/v6`](https://github.com/santhosh-tekuri/jsonschema) for JSON Schema Draft 2020-12 validation. The `ulc` binary exposes four subcommands: `ulc build-index <record.ulc>` (canonical index generator, with `--check` and `--stdout` modes), `ulc validate <record.ulc>` (schema validation, builder parity, source-file SHA-256 hash verification, computed conformance-level reporting with its enrichment roadmap, and the Product Achievements axis; `--json` for machine-readable output), `ulc from-sheet <bundle-dir|workbook.xlsx>` (a deterministic, offline converter from a manufacturer workbook to validated ULC records), and `ulc scope <record.ulc>` (the grading-scope manifest: a JSON document stating which blocks and graded items the rubric holds in scope for that record). Built-in releases ship platform binaries via GoReleaser. See `tools/validator/README.md` for build instructions and shipped-feature status.
