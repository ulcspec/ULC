...
- `hooks/pre-commit` is a shipped git pre-commit hook that mirrors the three CI guards plus a research-folder guard. Install it locally with `cp tools/hooks/pre-commit .git/hooks/pre-commit && chmod +x .git/hooks/pre-commit`. Optional — CI enforces the same checks on every pull request.
