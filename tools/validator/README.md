...
- [x] `ulc from-sheet <bundle-dir|workbook.xlsx>`: deterministic, offline converter from a manufacturer workbook (CSV bundle or native `.xlsx`) to validated ULC records (`--out`, `--assets`, `--allow-missing-files`, `--draft-out`, `--json`). Dual-unit fields on the records sheet accept either entry side: author the SI column or its Imperial companion column (never both for one field on one row), and the converter computes the other leaf per the published conversion policy (`docs/conversion-policy.md`, shipped in the release archive as `CONVERSION-POLICY.md`) with the authored value written verbatim. `--json` emits one machine-readable conversion report (`from_sheet_version` `1.0.0`, an additive-only contract independent of the CLI version and the ULC version line) carrying each record's `record_id`, authoring `pattern`, `status` (`written` / `draft` / `failed`), `conformance_level`, `warnings`, the output `path` or `draft_path` where a file was written, `missing_index_keys` naming any required index keys the builder could not derive, an `error` string when a record failed outside schema validation, and, for records that reached validation, `findings` in the same shape `ulc validate --json` emits, plus a run-level `summary` rollup counting the three outcomes; exit codes and the default human-readable output are unchanged. Workbook archives are bounded: entry count, per-part and total inflated size, and compression ratio all carry limits, and a workbook exceeding one is rejected with an error naming the limit. The CSV bundle path is unchanged. A reference that resolves outside the assets root, including by way of a symbolic link, is rejected. A reference to a file that is not present is an error unless `--allow-missing-files` is set, which stamps the 64-zero placeholder hash and marks the record a DRAFT: a draft is never written to `--out` and the run exits non-zero; pass `--draft-out DIR` to save drafts as `<record_id>.draft.json` for later completion.
  - **`record_id` is workbook-authored text.** It is schema-checked only once a record reaches validation, and `warnings` and `error` strings can embed workbook-authored filenames. Treat these as untrusted: never a filesystem path, a cache key, or a trust anchor, and escape them for whatever output context they land in. Angle brackets and ampersands are escaped in the emitted JSON, matching `ulc scope`.
- [x] Builder parity is included in `ulc validate` (stored `index` vs. computed)
- [x] File-reference SHA-256 hash verification when referenced files are reachable on the local filesystem: `source_files[].reference`, `product_family.cutsheet`, `emergency.photometry_reference`, `product_family.shared_warranty.conditions_document`, and `media[].reference` are checked on every run, and the attestation evidence references under `--verify-evidence`. A locally absent file is an INFO, an unreadable file is a WARNING, and a file whose SHA-256 does not match its declared hash is an ERROR that fails validation. A declared hash equal to the 64-zero placeholder that draft conversion stamps is flagged with a `source-file/placeholder-hash` WARNING whether or not the file is reachable locally; the warning never changes the exit code.
