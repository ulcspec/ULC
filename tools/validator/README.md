...
- [x] File-reference SHA-256 hash verification when referenced files are reachable on the local filesystem: `source_files[].reference`, `product_family.cutsheet`, `product_family.shared_warranty.conditions_document`, and `emergency.photometry_reference` are checked on every run, and the attestation evidence references under `--verify-evidence`. A locally absent file is an INFO, an unreadable file is a WARNING, and a file whose SHA-256 does not match its declared hash is an ERROR that fails validation.
