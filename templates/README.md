...
You do not set a conformance level. It is computed: `ulc build-index` grades the record from its populated fields and stamps the result into the generated index as `index.conformance_level`. The skeleton templates grade `core`; the level climbs to `standard` as the standard-level fields (measured values, provenance, an LM-79 attestation) fill in, and then to `full` once operating-point qualifiers (and a BUG rating for outdoor products) are added on top. A skeleton already ships an operating_point block, so it grades `core` only until the standard-level fields are present, not on operating-point alone. Records at full commonly carry deeper comprehensive data (TM-30 hue bins, method-backed lumen-maintenance projections, measurement uncertainty, and instrumentation metadata), which the validator reports as INFO observations rather than hard requirements.
