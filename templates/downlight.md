...
- **`index.conformance_level`** — generated, not hand-set. `ulc build-index` computes it from the fields you populate and stamps it into the index. A skeleton grades `core`; it climbs to `standard` once measured values, provenance, and an LM-79 attestation are in, and then to `full` once operating-point qualifiers (and a BUG rating for outdoor products) are added on top of those standard-level fields (the skeleton already ships an operating_point block, so it grades `core` until the standard-level fields are populated, not on operating-point alone). Comprehensive data (TM-30, method-backed lumen-maintenance projections, uncertainty, instrumentation depth) is reported as INFO observations, not required for `full`. Do not type it yourself; a hand-edited value fails the builder-parity check.
...
- **`shared_mechanical.lens_material: "cone_only"`** — used when the optic has a reflector cone and no separate lens. Change to `tempered_glass`, `glass`, `acrylic`, or `polycarbonate` if a lens is present.
...
- **`fixed_axes`** — the order-code axes that this specific scenario holds constant (catalog number, CCT, distribution, finish, voltage). Authors of multi-SKU records should read `docs/authoring-patterns.md` for how to express variable axes via `applicability.covered_axes` with a per-axis `CoveredAxis.derivation` rule (multiplier table, linear scaling, voltage-independence rationale).

### `electrical`

- **`input_power_w`** — from the IES file `[INPUT POWER]` keyword or the cutsheet's rated input. Use `value_type: "measured"` with `provenance.source: "ies"` and `provenance.attestation_ref` pointing at the LM-79 attestation ID when the IES is the source. Until a lab attestation is linked, the template placeholder uses `value_type: "rated"` with `source: "manufacturer_direct"`.
- **`driver_protocol`** — `0-10v`, `1-10v`, `dali`, `dali_2`, `phase_forward`, `phase_reverse`, `dmx`, `non_dimming`, and so on. Downlights are most commonly `0-10v` in North America and `dali` or `dali_2` in Europe. See `taxonomy.schema.json#/$defs/DimmingProtocol` for the full enum.
...
- **`symmetry_type`** — `symm_full` for axially-symmetric round downlights (the default); `symm_bi_0` for wall-wash downlights with bilateral symmetry about the C0-C180 plane; `symm_none` for fully asymmetric optics.
- **`luminous_opening_shape`** — `circular` for round apertures; `rectangular` for rectangular trimless; `rectangular_with_luminous_sides` if the optic emits through side walls. See `taxonomy.schema.json#/$defs/LuminousOpeningShape` for the full LM-63 enum.
