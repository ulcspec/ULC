...
- **`index.conformance_level`** — generated, not hand-set. `ulc build-index` computes it from the fields you populate and stamps it into the index. A skeleton grades `core`; it climbs to `standard` once measured values, provenance, and an LM-79 attestation are in, and to `full` once operating-point qualifiers (and BUG for outdoor products) are populated. Comprehensive data (TM-30, method-backed lumen-maintenance projections, uncertainty, instrumentation depth) is reported as INFO observations, not required for `full`. Do not type it yourself; a hand-edited value fails the builder-parity check.
