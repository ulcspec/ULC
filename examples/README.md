...
The four standard records sit one tier below full for the same structural reason: full certifies laboratory-report depth (measurement uncertainty, applied corrections, TM-30 fidelity, and either zonal lumens or a method-backed maintenance projection) that a published cutsheet does not carry. The Vode record is the lone example held at core, by a single missing field: it publishes no MacAdam SDCM step, which standard requires of a white-light fixture.
