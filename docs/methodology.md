...
- From accredited test reports: measured colorimetry (measured CCT, Duv, chromaticity, per-sample CRI, and full TM-30 per-hue-bin fidelity); measured electrical (input current, measured power factor and THD); measured alpha-opic and flicker values; package and luminaire lumen-maintenance test results and projections; thermal derating; chromaticity-shift projection; corrections applied; measurement uncertainty; deep operating-point qualifiers (drive current, ambient and case temperature, monitoring point, operating mode); and goniometer type.
...
The converter applies this ladder per value. Measured alpha-opic values select only the RP-46 family, measured flicker values only LM-90-20, IEEE 1789-2015, or NEMA 77-2017, package-maintenance values and the records-sheet manufacturer lumen-maintenance claim only LM-80 or TM-21, and measured photometry only LM-79. Automatic and explicit references obey the field's family boundary; the manufacturer-rated claim cannot be relabeled measured.
...
| Outdoor distribution type | `outdoor_classification.outdoor_distribution_type` | outdoor-site fixture whose markets do not explicitly exclude North America |
| Longitudinal range | `outdoor_classification.longitudinal_distribution_range` | outdoor-site fixture whose markets do not explicitly exclude North America |
| BUG rating | `outdoor_classification.bug_rating` | outdoor-site fixture whose markets do not explicitly exclude North America |
...
Two invariants keep the layer safe. First, every **gating-row** predicate reads only core fields (primary category, color tunability, indoor or outdoor, environment rating, technical region, driver protocol, mounting types), class-core fields, or an explicit applicability declaration. `product_family.markets` is the one applicability declaration: it is optional and ungraded, but a non-empty list that excludes `north_america` removes the three North American outdoor-classification rows. An absent or empty list preserves the earlier scope. The market-safety predicate additionally reads the attestations, which are themselves the core safety rule. The class profiles extend the invariant to class-core fields: a sign's `illumination_mode` and a powered emergency product's `power_source` are core-required for the class whose gating rows read them (through `signMode` and the battery trio), so those reads stay within that class's core tier; the battery trio gates at standard only for the dedicated-emergency and internally-illuminated classes, where `power_source` is core, and surfaces as enrichment for an externally-illuminated combo sign, where it is not. Grading walks the tiers low to high, so a predicate that decides a standard- or full-tier hard rule must not read a standard- or full-tier field, or adding one field could change which higher-tier rules apply and make the level order-dependent. The enrichment and observation rows are non-gating, so their applicability predicates may read the presence of a parent block and non-core fields within it (a flicker sub-field is suggested only when a flicker block is present; an emergency-mode nudge fires only when `emergency_role` names a dual-mode product); because they never move the level, reading a non-core block or field there is harmless. The reference tests assert the invariant behaviorally over the gating rows: stripping every standard and full field from a record must leave each gating predicate's value unchanged, including when `markets` includes or excludes North America. Second, each predicate is pure and total, so it never panics on a malformed record, because grading runs before full schema validation on the spreadsheet authoring path.
...
| Predicate | Reads (core or applicability fields) | True when | Gates |
| --- | --- | --- | --- |
...
| `outdoorSite` | `primary_category` | flood_area_site, roadway_street, walkway, bulkhead_wall_pack, sports_flood | joined with `northAmericanMarket` for outdoor distribution type, longitudinal range, and BUG (standard); LCS zonal (enrichment); legacy cutoff (observation) |
| `northAmericanMarket` | `product_family.markets` | absent, empty, or contains `north_america` | joined with `outdoorSite` for the three North American outdoor-classification rows |
...
The determination these predicates make is also exported in result form, by `ulc scope <record>`: it states which blocks and which graded items the rubric holds in scope for a record, and never the predicate logic that decided them, so a consuming tool reads the result rather than reimplementing the rubric. It covers the gating tiers only, and it is distinct from the record's `applicability` block, which declares the range of orderable SKU configurations.
...
The `claimed`-to-`documented` discriminator is an attached, unexpired evidence document, never a status token. An attestation is evidence-backed when it carries a `source_document_ref`, a file reference the schema guarantees to have a filename and a SHA-256 content hash. The hash is the integrity anchor a consumer can check; the `program` token names the qualification, and the optional `issuing_authority` field names the specific operator that issued it when a program runs under more than one (as EPDs do). `documented` is a record fact ("an evidence document is attached"), never a verification performed by ULC, which is the same trust boundary the safety gate observes. The `restricted_substances_declared` sibling flag records the RoHS, REACH, Prop 65, and similar restricted-substances programs the ledger declares; it sits beside the themes, never inside one, because restricted-substances compliance is a legal floor, not a prestige achievement. A manufacturer may publish the reference while withholding the document itself: when the withheld document is listed in `source_files`, which is how evidence documents are attached, a default `ulc validate` run reports one `source-file/not-found-locally` INFO at that `source_files` entry and the theme still reads `documented`, because the hash lets any consumer who later obtains the document verify it independently. `ulc validate --verify-evidence` extends the same byte check to the attestation's own `source_document_ref`, where an absent document stays INFO and a hash mismatch is an ERROR.
