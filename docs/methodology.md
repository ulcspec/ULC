...
**Explicit completeness.** Missing fields are explicit rather than hidden, and the computed conformance level (incomplete, core, standard, full) makes a record's completeness legible at a glance. When every manufacturer used their own notion of what belongs on a datasheet, gaps were invisible. A common structure where absence is stated, and where the level is computed from the data actually present, surfaces data-quality issues that no single proprietary format ever exposed.
...
There are four ordered tiers, `incomplete` < `core` < `standard` < `full`, sitting above a `none` floor reserved for a non-photometric record that cannot be indexed at all (one missing the three anchors: total luminous flux, input power, and primary category).

- **`incomplete`.** A real photometric record (it carries the three anchors, so it indexes) that has not yet met every core requirement. It still earns an index and a roadmap naming the core fields it is missing.
...
The market safety listing is region-aware: a North American record (technical region `120v_60hz_north_america`) must claim a North-American-recognized listing (UL, cUL, ETL, CSA, MET, a generic NRTL claim, or UL 1598); everywhere else any recognized listing, including CE, ENEC, or IEC 60598, satisfies the gate.
