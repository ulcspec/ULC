...
The converter's automatic selection applies this ladder per value. Measured alpha-opic values select only the RP-46 family, measured flicker values only LM-90-20, IEEE 1789-2015, or NEMA 77-2017, package-maintenance values only LM-80 or TM-21, and measured photometry only LM-79. Explicit references on supplementary fields and records-sheet photometry obey the same family boundary. The one narrow records-sheet exception lets the rated manufacturer lumen-maintenance claim name maintenance evidence explicitly; it does not apply to measured values or derived base references.
