...
- `ProductFamily.physical_dimensions` — block with slots for overall length / width / height / diameter, luminaire mass, linear mass per foot, lens width, ceiling aperture, recess depth, ceiling-thickness accommodation range, connection-cable length, driver dimensions, and EPA (pole-top outdoor). Uses DualUnitLength and DualUnitMass throughout.
