...
**Withholding the evidence document.** A manufacturer can document a theme while keeping the evidence document itself unpublished. The reference crosses the trust boundary; the bytes do not. Attach it exactly as the preceding paragraph describes, carrying the same filename and the same real SHA-256 hash of the withheld document in both the `source_files` entry and the attestation's `source_document_ref`. Give it a neutral publication name (for example `emergency-listing-evidence.pdf`) rather than the internal document name. A default `ulc validate` run then reports one `source-file/not-found-locally` INFO at the `source_files` entry (`/source_files/<i>/reference`), the record validates cleanly, and the theme still reads `documented`, because `documented` records that an evidence document is attached, not that its bytes are reachable. Once the file does sit beside the record, that same default run byte-verifies it: a matching hash is silent, and a mismatch is an ERROR. `ulc validate --verify-evidence` extends the identical check to the attestation's own `source_document_ref` pointer, where an absent document stays INFO and a hash mismatch is an ERROR. An attestation whose `source_document_ref` names a document that is not listed in `source_files` produces no finding on a default run; it is checked only under `--verify-evidence`. Choose the neutral filename deliberately: test-report filenames often name the laboratory, the product, the standard, and the test date, so a self-describing filename can disclose more than the manufacturer intends.
...
### `product_family.customization_openness`

An optional array inside `product_family` naming the axes of the family the manufacturer states are open to requests beyond the published order-code menu. The statement is cutsheet-level ink ("other color temperatures on request", a published modifications list), so it lives with the other family facts and is authored identically in every record of the family. It exists for the near-miss search: a specifier whose requirement falls outside the published menu can see the closest published record together with the axes its manufacturer states are open, instead of no match at all.

Each entry carries `axis` (from the closed `CustomizationAxis` vocabulary), optionally the manufacturer's own `axis_label` and on-request `statement`, and at least one of `published_in_ref` or `contact_reference`. Choosing between the two is mechanical: openness printed on a document cites that document (`published_in_ref`, a filename in the vocabulary of the record's `source_files` entries and the family `cutsheet`, its typical carrier); openness the manufacturer states without printing it carries the routing (`contact_reference`); a sheet that prints both a statement and a routing carries both. `axis_label` is required when `axis` is `other`, and `other` may repeat only with distinct labels; every other axis appears at most once. On a listed axis, `axis_label` may carry the manufacturer's own narrower name for the opening (a color option confined to cords, for example) without changing which token facets. An empty list is invalid: state at least one axis or omit the field. `contact_reference` names a role or department routing the manufacturer already makes public, never a named individual's personal contact details. `published_in_ref` names a published document; a non-public document's name can disclose more than the manufacturer intends, so the same neutral-naming caution that governs withheld evidence documents applies. The validator does not resolve `published_in_ref`: resolution against any one record's file manifest is a courtesy rather than a contract, and nothing in ULC checks that the named document actually carries the statement; the citation records where the manufacturer publishes it.

The marker declares openness only. It never enumerates what the custom choices are, never implies price, lead time, minimum quantity, or that any specific request will be accepted, and never widens the record's applicability: a configuration obtained through a request is outside the covered range and inherits none of the record's measured values, derivations, or attestations. When the printed line itself enumerates values, cite the document and elide the list: a printed "custom lengths from two to twelve feet on request" (a constructed example) is authored as the entry's axis plus `"statement": "Custom lengths on request."` with the sheet cited, or with `statement` omitted entirely; the enumerating clause never enters the record. ULC records openness only; a refusal has no encoding, because the printed convention is an on-request statement rather than a refusal. `contact_reference` carries the same meaning as the attestation verification type `requires_manufacturer_confirmation`: the manufacturer confirms each request per project.

**How consumers should treat customization openness.** An openness entry is author-asserted text: display it attributed, dated by the record's `record_status_as_of` where the record carries one (a record without that date states openness with no stated date), and apply your own validation before rendering or linking a routing string. A consumer may facet on the axis tokens and return the record as a near match, visibly flagged as request-required, ranked below covered matches, and never merged into the covered set. Only listed axes may be rendered as open: an absent axis is unstated, neither open nor refused. A consumer may distinguish printed openness (`published_in_ref`) from stated-only openness, though the distinction is not a quality ranking. A consumer must never apply any measured value, derivation, computed grade, or attestation of the record to a custom variant; never compose or extend an order code for one; never parse a `statement` for values or paraphrase it as a commitment; and never infer price, lead time, minimum quantity, feasibility, or that any particular region, representative, or project size will honor a request. Tokens are deliberately coarse: an opening whose statement names cord length still facets as `dimensions`, and the statement carries the narrowing for human readers, so a token never licenses machine-readable openness on everything its bound surface spans. Sibling records of one family should carry identical lists; on conflicting siblings, treat the family's statement as unresolved rather than merging.

**Token bindings (authoritative).** This table is the authoritative copy of the token-to-surface bindings; the taxonomy's enum description is its summary. A token names the axis, never any value on it. The binding class states what the anchor is: a published-values surface lists or carries the published values the axis ranges over; a property block is the typed block where the axis's data lives; a class vocabulary is a closed vocabulary typing the axis without listing a menu; a measured surface carries the tested value the axis varies; the escape hatch has no anchor, and `axis_label` carries the name.

| Token | The axis, in plain terms | Bound ULC surfaces | Binding class |
| --- | --- | --- | --- |
| `cct` | correlated color temperature | `colorimetry.nominal_cct_k`; `NominalCCT` | published-values surface |
| `cri` | color rendering | `configuration.tested_axes.cri_tier`; `CriTier` | published-values surface |
| `finish` | any published color or finish axis, cordsets included | `product_family.shared_mechanical.finish_color_options` | published-values surface |
| `optic` | distribution and beam | `photometry.distribution_type`, `photometry.beam_family`; `DistributionType`, `BeamFamily` | published-values surface |
| `mounting` | how the fixture attaches (canopy, pole, wall, recessed frame) | `product_family.mounting_types`; `MountingType` | published-values surface |
| `mechanical` | structure, housing, material (a custom housing, a non-standard material) | `product_family.shared_mechanical` | property block |
| `dimensions` | physical size (lengths, heights, overall dimensions) | `product_family.physical_dimensions` | property block |
| `driver` | the driver itself (a non-standard driver, integral versus remote) | `electrical.control_gear_type`; `ControlGearType` | class vocabulary (hardware, not signaling) |
| `controls` | which control protocol the fixture speaks | `electrical.driver_protocol`; `DimmingProtocol` | class vocabulary (signaling, not hardware) |
| `light_source` | a different kind of source or tuning behavior, not a different fixed color temperature (which is `cct`) | `configuration.tested_axes.color_tunability`; `ColorTunabilityCapability` | class vocabulary |
| `output` | lumen output | `photometry.total_luminous_flux_lm`, `photometry.led_module_luminous_flux_lm` | measured surface |
| `emergency` | factory emergency-power options | `emergency.emergency_role` and the `emergency` block; `EmergencyRole` | property block |
| `other` | the manufacturer's own labelled axis | none; `axis_label` carries the name | escape hatch |

One trap the field names set: they cross. `controls` anchors to `electrical.driver_protocol` and `driver` anchors to `electrical.control_gear_type`; choose by what the printed line is about, never by the word inside the field name.

Authoring rules the table implies: pick the token whose bound surface carries the axis's data; never relabel a listed axis through `other`, which defeats the cross-manufacturer facet the closed vocabulary exists for; file a genuinely unlisted axis as `other` with the manufacturer's own label, which is also the demand signal that mints the next token. Cord colors file under `finish`; cord and cable lengths file under `dimensions`; a custom housing or a non-standard material files under `mechanical`. A different fixed color temperature files under `cct`; tunable-white, dim-to-warm, or color-mixing behavior files under `light_source`.

```json
"product_family": {
  "customization_openness": [
    { "axis": "cct", "statement": "Additional color temperatures available on request.",
      "published_in_ref": "family-cutsheet.pdf" },
    { "axis": "mounting", "published_in_ref": "family-cutsheet.pdf",
      "contact_reference": "requests@manufacturer.example" },
    { "axis": "other", "axis_label": "acoustic options",
      "contact_reference": "requests@manufacturer.example" }
  ]
}
```

**The boundary with the ordering menu.** Openness and coverage are complementary statements. Any per-axis fact about the published menu belongs on the applicability block (`covered_axes`); any fact about the boundary of the menu belongs on `customization_openness`. The test is menu membership: if a specifier can order a value by writing a published catalog number, it is menu territory; if they cannot, it is openness territory. Some sheets publish a custom-placeholder order code (a specify-length token, a consult-factory code): a placeholder names openness, never a value and never a complete order. On an axis that does not affect photometry, the applicability block can carry the same openness as an authored covered-value placeholder, as the shipped corpus already legitimately does (the Vode pendant record covers an "any" arm-cable-length value under a photometrically-irrelevant rationale); the record then carries both statements of one openness, the covered value making it orderable and the marker making it searchable. On an axis where photometry depends on the value, no placeholder ever counts as covered, and the openness statement lives here alone. `CustomizationAxis` is customization-scoped: the ordering grammar's vocabulary is separate, and neither reuses the other's.
...
`media` is deliberately separate from `source_files`, and the boundary is what the entry is: a picture of the product belongs in `media`, and a document belongs in `source_files`. A source file is a document the record derives data from, and its `file_type` can serve as the provenance source for a traced value; a media asset documents appearance and geometry for presentation, carries no `SourceFileType` token, and can never back a measured value. The schema enforces the picture side mechanically: `media` admits image formats only, so a document format can never be declared there. A PDF drawing is filed in `source_files` like any other document, and an image file that is really a scanned document (a photographed certificate, for example) is also filed in `source_files`, under its document type. When authoring a dimensional drawing, prefer SVG, then PNG.

**How consumers should treat media.** Verification of a media asset is three-state. An asset whose bytes are present and match the declared hash is verified and renders normally. An asset that cannot be verified, because the file is absent locally or a URL copy has not been fetched and hashed, is unverified: a consumer may render an unverified `product_photo` or `application_photo` with a visible unverified marking, and must not render an unverified `dimensional_drawing`, because wrong bytes on a drawing are a dimensional data hazard rather than a cosmetic one. A consumer that fetches an asset from its `url` verifies it the same way: hash the fetched bytes and compare before trusting them. The third state is absence: the record declares no asset for a role, which is different from a declared asset whose file is missing. A default `ulc validate` run byte-verifies every media entry whose file sits beside the record: a matching hash is silent, a mismatch is an ERROR, an absent file is an INFO, and an unreadable file is a WARNING, the same semantics the walk applies at the record's other default-verified reference sites: the source files, the family cutsheet, the emergency photometry reference, and the family warranty-conditions document.
