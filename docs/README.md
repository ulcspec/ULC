...
- `conversion-policy.md`: the dual-unit conversion policy, covering the five unit families, the conversion rule and rounding each uses, the authored-value-verbatim rule, and the single-unit exceptions. Shipped in every release archive beside the `ulc` binary.
