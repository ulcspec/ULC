...
Two columns carry the load. **Category** sorts each badge into one governance family. **Conformance role** states whether the reference grader reads the token. It is one of two roles: `tracked` (the token is recorded for catalog and search and never affects the conformance level) or `core-eligible safety listing` (its presence can satisfy the grader's core safety gate). The safety-listing role is region-aware, mirroring the region-conditional gate, so a row may be qualified: `core-eligible safety listing (North America)` counts only toward a North-American record's gate, `core-eligible safety listing (outside North America)` counts toward every other region's gate but not North America's, and an unqualified `core-eligible safety listing` is accepted by any region. The verification standing of any claim, genuine or not, lives separately in the attestation's own `AttestationStatus` field (`claimed`, `verified`, `listed`, `audited_member`, `provisional_member`, `expired`, `withdrawn`, `not_applicable`).

**Conformance grading checks for the presence of a self-asserted attestation claim, not third-party verification of it, so a ULC conformance level is a data-completeness grade, never a safety certification (verification state lives in the attestation's own `AttestationStatus`, never in the grade; see [the trust boundary in methodology.md](methodology.md#the-conformance-rubric)).**
...
| `met_listed` | MET Listed (MET Labs) | Product safety listing by MET Laboratories, the first OSHA-recognized NRTL. | MET Labs / Eurofins (NRTL) | safety-listing | `core-eligible safety listing` |
...
| `tuv` | TUV mark | Product safety certification by a TUV body (TUV Rheinland, TUV SUD, TUV Nord). | TUV against IEC/EN luminaire safety standards | safety-listing | `core-eligible safety listing (outside North America)` |
| `cb_scheme` | IECEE CB Scheme | CB Test Certificate and report enabling mutual recognition of product-safety test results across IECEE member bodies. | IECEE CB Scheme (IEC standards) | safety-listing | `core-eligible safety listing (outside North America)` |
...
| `ce` | CE marking | Manufacturer's declaration of conformity with applicable EU directives (Low Voltage, EMC, RoHS) for sale in the EEA. | EU CE marking framework | safety-listing | `core-eligible safety listing (outside North America)` |
| `ukca` | UKCA marking | UK Conformity Assessed mark; the Great Britain equivalent of CE marking post-Brexit. | UK product-safety regulations | safety-listing | `core-eligible safety listing (outside North America)` |
| `enec` | ENEC mark | European Norms Electrical Certification, a pan-European third-party safety mark to EN standards. | ENEC scheme (EN 60598 family) | safety-listing | `core-eligible safety listing (outside North America)` |
| `iec_60598` | IEC 60598 | International safety standard for luminaires (general and particular requirements). | IEC 60598 series | safety-listing | `core-eligible safety listing (outside North America)` |
| `nom` | NOM mark | Mexican official-standard safety certification (Norma Oficial Mexicana) required for sale in Mexico. | NOM (Mexico) | safety-listing / market-access | `core-eligible safety listing (outside North America)` |
| `ccc` | CCC mark | China Compulsory Certification required for many products sold in mainland China. | CCC (China) | safety-listing / market-access | `core-eligible safety listing (outside North America)` |
| `rcm_australia` | RCM | Regulatory Compliance Mark for electrical safety and EMC in Australia and New Zealand. | RCM / ACMA + electrical regulators | safety-listing / market-access | `core-eligible safety listing (outside North America)` |
| `saa_australia` | SAA approval | Standards Australia approval mark for electrical equipment safety. | SAA / Australian electrical safety scheme | safety-listing / market-access | `core-eligible safety listing (outside North America)` |
| `kc_korea` | KC mark | Korea Certification mark for electrical and electronic equipment safety. | KC (Korea) | safety-listing / market-access | `core-eligible safety listing (outside North America)` |
...
Each theme resolves to one of three states, `none`, `claimed`, or `documented`, defined in [the two grading axes in methodology.md](methodology.md#the-two-grading-axes-completeness-and-achievements); the program-to-theme map below is this glossary's lane.
...
- **Origin claims outside the domestic_content theme**: `taa` and `country_of_origin` stay unthemed by design. TAA eligibility can be satisfied by non-US manufacture in a designated country, so it is not a domestic-content claim; country of origin is a neutral provenance fact, not an achievement. (`baa`, `baba`, and `american_iron_and_steel` are now the `domestic_content` theme, mapped above.)
- **Controls** (`dlc_nlc`, `ntcip_1213`) are held for a future theme.
