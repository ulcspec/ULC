...
4. **Missing provenance history.** If the PIM was hand-populated over years with no source tracking, retain and hash a structured export snapshot and use `{source: "manufacturer_data_export", method: "extracted"}` for values it supplies. Use `{source: "manufacturer_direct", method: "transcribed"}` only for a manufacturer statement with no attachable file behind it. Plan a follow-up pass to enrich provenance when lab reports get linked.
