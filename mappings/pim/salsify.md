...
**Certificate and disclosure assets map to attestations.** A UL, DLC, EPD, or Declare asset is more than a source file: map it to an `attestations[]` entry with its `program` token. A program assigned to an achievement theme (DLC energy, EPD embodied-carbon, Declare material-health) contributes `claimed` on the token alone to `index.achievements` and reaches `documented` only when you attach the certificate as `source_document_ref` (the streamed asset's `{filename, sha256}`); an unthemed UL safety listing feeds `index.attestation_programs` but no achievement theme. Carry `valid_until` on any dated program so `ulc validate --expiry` can preview its expiry; a `sustainability_declaration`'s dated `expiration_date` belongs on that block, never on an attestation. RoHS/REACH-class programs also populate the computed `index.restricted_substances_declared` flag.
