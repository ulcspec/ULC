...
- Exit codes: 0 when a manifest is emitted or `-h` is given; 1 when the record cannot be read, parsed, or used as a ULC record, or the manifest cannot be written, always with a diagnostic on stderr; 2 on a usage error. A read, parse, or encode failure writes nothing at all to stdout, since the document is encoded in full before any of it is written. The subcommand runs no schema validation and reads nothing but the record, so it is report-time only.
