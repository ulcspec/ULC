...
- `shared_warranty.conditions_document` is a hashed, dated reference to the published warranty-conditions document, so exclusions and conditional extensions travel with the headline number. The free-text `conditions_reference` is deprecated in its favor and is scheduled for removal at v2.0.0; it remains valid throughout the 1.x line.
