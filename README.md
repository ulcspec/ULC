...
- **ETIM** (ElectroTechnical Information Model) provides a widely adopted classification vocabulary for product attributes in electrotechnical wholesale. Individual ULC schema fields cite the relevant ETIM feature identifiers inline in their descriptions where a direct correspondence exists (for example `EF001596` on housing material); the separate compiled crosswalk at `mappings/etim-crosswalk.md` that would enumerate the full set in one place is still planned and has not yet landed.
