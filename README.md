...
| `docs/` | Narrative documentation: `how-it-works.md` (end-to-end overview), `methodology.md` (design rationale), `authoring-patterns.md` (the manufacturer authoring patterns, the exit-sign/emergency product class, and architectural primitives), `compliance-attestation.md` (the attestation-program glossary and achievement-themes appendix), and `conversion-policy.md` (the dual-unit conversion policy: the five unit families, the conversion rule and rounding each uses, and the single-unit exceptions). |
